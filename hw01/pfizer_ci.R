...
z <- qnorm(0.95, mean = 0, sd = 1) # FILL IN HERE (normal quantile to be used in 95% CI)
CI <- c(pi.hat - z*se.pi.hat, pi.hat + z*se.pi.hat )## FILL IN HERE (95% CI)
print(CI)


### Simulate sampling distribution (i.e., repeated experiments) of pi estimator ------
n.simul <- 1000
set.seed(4321) # re-run every time you want to re-simulate!
...
mu_tilde_b <- rep(NA_real_, times = B)
for(b in 1:B){
  Y <- sample(y, size = n.simul, replace = TRUE) # nonparametric bootstrap sampling
  mu_tilde_b[b] <- mean(Y)
}
se_mu_hat <- sqrt(var(mu_tilde_b))
se_mu_hat # 0.06507467
